...
std::string DBPath = "../data/experiment3";
...
void AddDeltas(lightgraph::LDB& db) {
    ...
    for(lightgraph::__vertex_id_t src = 0; src < 1000; src++) {
        for(lightgraph::__vertex_id_t dst = 0; dst < 10000; dst++) {
            t = random() % 5 + 1;
...
void TestGetOutVDuringWithIndex(lightgraph::LDB& db) {
    ...
    for(lightgraph::__time_t upper = 2; upper <= 10; upper += 2) {
...
void TestGetOutVAtWithIndex(lightgraph::LDB& db) {
    ...
    lightgraph::__vertex_id_t dst;
    for(lightgraph::__time_t t = 2; t <= 10; t += 2) {
        time_start = std::chrono::steady_clock::now();
        for(lightgraph::__vertex_id_t src = 0; src < 100; src++) {
